...
`;
  ...
  ensureDefaultDictionary() {
    if (localStorage.getItem("customDictionary")) return;
    const defaultTerms = [
      // Coding terms
      "algorithm",
      "api",
      ...
      // Brand voice (for biasing recognition)
      "Gritty",
      "Rebellious",
      "Unfiltered",
      "Fiercely Independent",
      "Raw and Direct",
      "Short, Heavy-Hitting Sentences",
      "Mechanical Metaphors",
      "Biker Lifestyle Vocabulary",
      "Total Authenticity",
      "Dirty Hands Respect",
      "Zero Patience for 'Garage Queens'",
    ];

    try {
      localStorage.setItem("customDictionary", JSON.stringify(defaultTerms));
      logger.debug("Default custom dictionary initialized", { count: defaultTerms.length }, "transcription");
    } catch (error) {
      logger.debug("Failed to initialize default dictionary", { error: error.message }, "transcription");
    }
  }
